 ...
 * InsightWidget shape the UI renders. This is the REST equivalent of what
 * generateInsights.ts does for the live SSE stream. The REST payload has no
 * per-chart description (only the insight-level insight_text) and no
 * source_urls, so description maps to insight_text and generation carries
 * only codeact_parts. `insightId` links every chart back to its parent
 * insight (the id the dashboards API references).
 */
