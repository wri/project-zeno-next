...
//
//  • Side panel (Analyses) — a full-height docked column, opened/closed on
//    its own.
//  • AI chat — either a floating, draggable card (default) or, when
//    full-sized, a full-height column docked to the LEFT of the side panel.
//
// Docked columns are in-flow (they shrink the content beside them); the
// floating chat is an absolute overlay.
...
export default function DashboardWorkspace({
  ...
  return (
    <Box h="100%" minH={0} position="relative" overflow="hidden">
      <Flex h="100%">
        ...
        {/* Dashboard content. */}
        <Box flex="1 1 auto" minW={0} h="100%" minH={0}>
          {children}
        </Box>
      </Flex>

      {/* Floating chat — overlay, only when not full-sized. Offset right of
          the side panel (when open) so it doesn't cover it by default. */}
      {!chatMaximised && (
        <Box
          ref={constraintRef}
          position="absolute"
        ...
        >
          ...
          >
            ...
            >
              <DashboardChatCard floating dragControls={dragControls} />
            </Box>
          </motion.div>
        </Box>
      )}
