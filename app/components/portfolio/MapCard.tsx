...
// Read-only maplibre embed rendering the AOI polygon for an area dashboard.
// Falls back to a green tinted SVG card when the geometry snapshot is
// missing — same shape the wireframe shows.
export default function MapCard({ aoi, height = 180 }: Props) {
  ...
  if (!aoi.geometry) {
    // Fallback when we couldn't snapshot a polygon at pin time.
    return (
      <Box
        bg="green.subtle"
        border="1px solid"
