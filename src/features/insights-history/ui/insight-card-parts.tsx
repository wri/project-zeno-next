...
/**
 * Detail view for one analysis: every chart at once, one card under the next,
 * with no pager. An analysis's charts are one finding — tree cover loss beside
 * the emissions it caused, the LGMS tree beside its time series — and paging
 * put half of each pair behind an arrow.
 *
 * An LGMS analysis arrives as four charts, three of which are the same time
 * series at different roll-ups; they fold into the one the DETAIL pill selects,
 * so two cards show rather than four. The pane's charts carry backend UUIDs
 * rather than the `{insightId}-chart-{n}` ids, so the roll-ups are found within
 * this one analysis and keyed on its id (see `netFluxRollups`).
 */
export function InsightGroupDetail({
  ...
  return (
    <Box w={`${CATALOG_CARD_WIDTH_PX}px`} maxW="100%" flexShrink={0}>
      <BackToAnalyses onBack={onBack} />
      <Stack gap={4}>
        {widgets.map((widget) => (
          <Box key={widget.id ?? widget.title}>
            {/* The pane is this card's shell, so the pills sit above it —
                `WidgetMessage` is mounted `inWorkspace` and suppresses its
                own inline copy. */}
            <Box mb={2}>
              <InsightChartPills
                widget={widget}
...
/** The pane's way back out of a detail view. */
function BackToAnalyses({ onBack }: { onBack: () => void }) {
  return (
    <Button
      variant="ghost"
      size="xs"
