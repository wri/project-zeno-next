 ...
 *
 * Defined here (analysis slice's `model` segment) rather than in the framework-free service because its vocabulary is
 * `InsightWidget` — a view-model type that belongs to the driving side. The service stays clean: it returns an
 * `AnalysisResult` and knows nothing about where widgets land.
 *
