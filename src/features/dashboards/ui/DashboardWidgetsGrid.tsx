...
export default function DashboardWidgetsGrid({
  ...
  return (
    <Box css={{ containerType: "inline-size", containerName: "widgets-grid" }}>
      <Grid
        templateColumns="1fr"
        css={{ [TWO_COLUMN_QUERY]: { gridTemplateColumns: "repeat(2, 1fr)" } }}
        gap={4}
      >
        {cells.map((cell, i) => {
          ...
          return (
            <GridItem
              key={cell.key}
              // Tracks may shrink below the cards' intrinsic minimum near the
              // two-column threshold; cards clip internally instead of forcing
              // the grid wider than the page.
              minW={0}
              css={
                size === "double"
                  ? { [TWO_COLUMN_QUERY]: { gridColumn: "span 2" } }
                  : undefined
              }
            ...
            >
              {widget.widget_type === "text" ? (
                ...
                <DashboardWidgetCard
                  ...
                  onRemove={() => {
                    // A chart card hides just its chart from the widget's
                    // shown set; the widget is deleted only when its last
                    // chart goes (or for placeholder cells with no chart id).
